...
- (void)userLoginByPhoneNumber:(NSString *) phoneNumber andVerifyCode:(NSString *) verifyCode andCallback:(void (^)(NSDictionary *resultDic)) callback{
    ...
    NSString *url_request = [NSString stringWithFormat:@"%@%@",URL_REQUEST,URL_REQUEST_SESSION_LOGIN_VERIFY_CODE];
    
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params setObject:phoneNumber forKey:@"phoneNumber"];
    [params setObject:verifyCode forKey:@"verifyCode"];
    [params setObject:VERIFY_CODE_VERIFY_CODE_LOGIN forKey:@"verifyTypeKey"];
    ...
    [requestHelper postRequestAsynchronousToUrl:url_request byParamsDic:params needAccessToken:NO andCallback:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        if (data) {
            ...
            if (errorMessage) {
                ...
                [self saveUserInfo:jsonDic];
                
                //成功，跳转到修改密码
                [resultDic setObject:IdentifyNameChangePasswordViewController forKey:RESULT_KEY_JUMP_PATH];
            }
        }else{
            [resultDic setObject:RESPONSE_ERROR_MESSAGE_NIL forKey:RESULT_KEY_ERROR_MESSAGE];
...
- (void)resetPassword:(NSString *) password ByAccessTokenAndCallback:(void (^)(NSDictionary *resultDic)) callback{
    ...
    ///api/v1/session/password/resetByAccessToken
    NSString *url_request = [NSString stringWithFormat:@"%@%@",URL_REQUEST,URL_REQUEST_SESSION_PASSWORD_RESET_BY_TOKEN];
    ...
    BMRequestHelper *requestHelper = [[BMRequestHelper alloc] init];
    [requestHelper postRequestAsynchronousToUrl:url_request byParamsDic:params needAccessToken:YES andCallback:^(NSData *data, NSURLResponse *response, NSError *error) {
